...
public class GPCoreRF extends GPProblem {
	...
	static String baseQueryPath = utilidades.ProjectSetup.ranksBasePath;
...
//	static ModeloVetorial MV = new ModeloVetorial();
	...
	public GPCoreRF(GPConfiguration a_conf)
	...
	public static void main(String[] args)
		...
		config.setFitnessFunction(new GPCoreRF.FormulaFitnessFunction());
		config.setStrictProgramCreation(true);
		GPProblem problem = new GPCoreRF(config);
	...
	extends GPFitnessFunction {
		...
		public static double computeRawFitness(final IGPProgram ind) {
			...
			for(int i = 0; i < numberOfQueries; i++){
				List<String> imagensConsulta = new ArrayList<String>();
				for(int j = 0; j < 5; j++){
					//System.out.println("=================\nIteração " + (String.valueOf(j+1)));
					imagensConsulta.add(String.valueOf(imagensSelecionadas[i]));
					
					//for(String consultaq : imagensConsulta){
					//	System.out.println(consultaq);
					//}
					
				//	System.out.println(imagensConsulta.size() + " imagens selecionadas como consulta");
					ranks = new HashMap<String, Map<String, Float> >();
					
					menoresMaiores = new HashMap<String, float[]>();
					for (String key : queries.keySet()) {
						//System.out.println("Combinando ranks do descritor " + key);
						List<Map<String, Float>> consultasRanks = new ArrayList<Map<String,Float>>();

						for(int k = 0; k < imagensConsulta.size() - 1; k++){
							consultasRanks.add(normalizaUmRank(recuperaRank(new File(baseQueryPath + "/" + key + "/" + String.valueOf(imagensConsulta.get(k).replace(".jpg","")) + ".txt"))));			
						}	
						consultasRanks.add(normalizaUmRank(recuperaRank(new File(queryPath + "/" + key + "/" + String.valueOf(imagensSelecionadas[i]) + ".txt"))));
						//System.out.println(consultasRanks.size() + " ranks a ser combinados");
						//System.out.println("consulta: " + imagensSelecionadas[i]);
						ranks.put(key, RF.combinaRanks(consultasRanks));
					...
					vetorFitness[i] = avaliaRank(novoRank, imagensSelecionadas[i], imagensConsulta);
					if(vetorFitness[i] < 1){
						imagensConsulta = RF.selecionaRelevantesSimulado(novoRank, String.valueOf(imagensSelecionadas[i])); //selecionaRelevantesSimulado(novoRank, String.valueOf(imagensSelecionadas[i]));
	...
	/*	Map<String, Float> expandeRank(Map<String, Float> rank, int k){
		...
		*/
		...
		static Map<String, Float> normalizaUmRank(Map<String, Float> rank){
